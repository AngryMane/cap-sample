...
/** 
 *  @class SampleClient 
 *  @brief This class provides the event interface from client to server, and vise-versa.
 */
class SampleClient {
private:
  ...
  protected:
    ...
    /**
     * @fn pushMessage
     * @brief When the server pushes back `pushMessage`, then this function will be called.
     */
...
public:
  ...
  /**
   * @fn start
   * @brief Start event loop. This is a blocking function till the end of event loop.
   */
  void start();
...
private:
  ...
  // These are the contact point with the server when pushing back any events from the server.
